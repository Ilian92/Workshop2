<x-admin-layout title="Nouveau pilier" subtitle="Créer un nouveau pilier">
    <div class="bg-white rounded-lg shadow-sm">
        <div class="p-6 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900">Créer un nouveau pilier</h3>
        </div>
        
        <form action="{{ route('admin.piliers.store') }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <textarea name="descriptionCourte" id="descriptionCourte" rows="3" 
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent" 
                              required>{{ old('descriptionCourte') }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="6" 
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
